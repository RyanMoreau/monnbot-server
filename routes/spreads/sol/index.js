...
if (!db) {
	console.log("SOL DB Error.");
} else {
	console.log("SOL Listening.");
}

// Create & Post SOL Data
module.exports.createSpread = function () {
	cron.schedule(" */2 * * * * * ", () => {
		let perp = `${process.env.SERVER_URL}/markets?market_name=sol-0924`;
		let usd = `${process.env.SERVER_URL}/markets?market_name=sol/usd`;
		...
		axios.all([requestusd, requestperp]).then(
			axios.spread((...res) => {
				...
				let askSpread =
					...
					100;
				axios
					.post(`${process.env.SERVER_URL}/spreads/sol`, {
						id: i,
						bid: bidSpread,
						ask: askSpread,
					})
