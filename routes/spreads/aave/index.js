...
if (!db) {
	console.log("AAVE DB Error.");
} else {
	console.log("AAVE Listening.");
}

// Create & Post aave Data
module.exports.createSpread = function () {
	cron.schedule(" */2 * * * * * ", () => {
		let perp = `${process.env.SERVER_URL}/markets?market_name=aave-0924`;
		let usd = `${process.env.SERVER_URL}/markets?market_name=aave/usd`;
		...
		axios.all([requestusd, requestperp]).then(
			axios.spread((...res) => {
				...
				let askSpread =
					...
					100;
				axios
					.post(`${process.env.SERVER_URL}/spreads/aave`, {
						id: i,
						bid: bidSpread,
						ask: askSpread,
					})
...
// Save aave Data in Mongo
module.exports.postSpread = async function (req) {
	const spread = new aave({
		bid: req.body.bid,
		ask: req.body.ask,
	});
	await spread.save();
